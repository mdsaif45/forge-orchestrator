...
export const IPC_CONTRACT = {
   ...
   *
   * Kept alongside `provider:chat` rather than replacing it. A request/response
   * channel cannot emit partial text, and converting the existing one would
   * force every caller to handle chunks — including callers that only want the
   * finished string. The chunks arrive on the one-way `provider:chunk` channel,
   * the same shape the terminal and workflow log already use.
   *
   * `streamId` is generated by the caller so two concurrent replies cannot be
   * interleaved into one transcript.
   */
  /**
   * A tool-using turn: the model reads and edits the project itself.
   *
   * Distinct from `provider:chatStream` because the shape of the work differs.
   * A chat turn streams prose; this one runs rounds of tool calls whose progress
   * is reported on `provider:chunk` as `tool` chunks, and whose answer arrives
   * whole at the end.
   *
   * `projectId` rather than a path: the workspace is resolved in main from the
  ...
  'provider:agentTurn': {
    ...
    response: z.strictObject({
      ...
      content: z.string(),
      reasoning: z.string(),
      /** What the model actually ran, in order — the turn's audit trail. */
      toolsUsed: z.array(z.strictObject({ name: z.string(), ok: z.boolean() })).readonly(),
  ...
  'provider:chatStream': {
    ...
    // Resolves when the stream is finished, carrying the assembled text so a
    // caller need not re-accumulate what it already received.
    response: z.strictObject({
      ok: z.boolean(),
      content: z.string(),
      reasoning: z.string(),
 ...
 *
 * `kind` separates a model's visible reasoning from its answer. Some models emit
 * both — measured with a local Qwen build, which wraps reasoning in `<think>`
 * tags — and folding them together would bake the reasoning into the saved
 * message, where it cannot be told apart later.
 */
