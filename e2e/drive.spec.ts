...
test('isolated real run', async () => {
  ...
  await page.waitForTimeout(2500)
  await page
    .getByRole('button', { name: /start work/i })
    .nth(2)
    .click({ timeout: 20_000 })
  await page.waitForTimeout(2000)
  const tmplLine = (await page.locator('body').innerText())
    .split('\n')
  ...
  for (let i = 0; i < 200; i += 1) {
    ...
    const line = t
      .split('\n')
      .filter((l) => /Halted|Iteration|Result:|VERIFICATION|STAGE START|DONE|complete/.test(l))
      .slice(-4)
      .join(' ~ ')
      .slice(0, 300)
    if (line !== last) {
      console.log(`[t=${i * 6}s] ${line}`)
      last = line
    }
    if (/Halted|Workflow complete|DONE/.test(t)) {
      console.log(`[stop] ${i * 6}s`)
      break
    }
  }
